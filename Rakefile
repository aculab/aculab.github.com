...
task :post do
  ...
  Dir.mkdir("assets/images/#{date}-#{slug}")
  puts "Creating photo directory for post: images/#{date}-#{slug}"
  open(filename, 'w') do |post|
    ...
    post.puts "<!-- Photos will be in /assets/images/#{date}-#{slug}/ -->"
    post.puts "<!-- <img id=\"aboutPhoto\" src=\"/assets/images/#{date}-#{slug}\"/> -->\n"
    post.puts "<!-- Template for youtube video embed -->"
    post.puts '<!-- <object class="video" type="application/x-shockwave-flash" data="http://www.youtube.com/v/VID_ID&amp;hl=en&amp;fs=1&amp;rel=0" width="560" height="315"><param name="movie" value="http://www.youtube.com/v/VID_ID&amp;hl=en&amp;fs=1&amp;rel=0" /><param name="allowFullScreen" value="true" /></object> -->'
    post.puts "<!-- Link reference for my old brain -->"
    post.puts '<!-- <a href="" target="_blank"></a> -->'
  end
end # task :post
